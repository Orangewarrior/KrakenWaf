...
| `--max-inspection-ms` | `0` (disabled) | Per-request wall-clock cap on WAF inspection (ms), shared across early/body/HPP/open-redirect phases. When set and elapsed, inspection blocks fail-closed with a deadline finding and queues a JSONL event under `logs/filter/deadline.jsonl`. `0` disables the deadline. Also configurable via `Max_inspection_ms` under `global-options` in [conf/filter.yaml](conf/filter.yaml) |
...
| `--max-per-ip-body-bytes` | `209715200` (200 MiB) | Per-peer ceiling on bytes from in-flight request bodies, keyed by the direct TCP peer rather than trusted-proxy `effective_ip`. Prevents one peer from saturating the global body buffer. Also configurable via `max_per_ip_body_bytes` in [conf/ratelimit.yaml](docs/rate_limit.md) |
