...
| `--wal-mode` | `sqlite` | Persistence backend for the local rate-limiter snapshot: `sqlite` (inspectable WAL) or `postcard` (atomic-rename binary, ~10–50× faster; `bincode` accepted as a deprecated alias). Ignored when using Redis. See [docs/rate_limit.md](docs/rate_limit.md) |
