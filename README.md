...
> **2.29.0 — Hardening, Performance & Memory-Pressure Refactor**.
> Broad refactor across the request/response pipeline, rate limiter, TLS
> stack, and detection engines. Builds on the 2.28.0 inflight-byte
> accounting with deeper changes:
>
> * **Drastically lower default buffer caps**: `--max-body-bytes` and
>   `--max-upstream-response-bytes` default to **8 MiB** (previously 100 MiB),
>   tunable through `rules/cmc/config.yaml :: memory-limits`.
>   `--max-connections` is now RAM-aware (≈ 1 connection per 2 MiB, clamped to
>   `[64, 4096]`).
> * **Body decompression before inspection** (gzip / br / deflate / zstd)
>   with a zip-bomb expansion-ratio guard — closes the trivial
>   `Content-Encoding: gzip` evasion.
> * **`multipart/form-data` parser**: each part is extracted and inspected
>   independently so payloads can no longer hide behind boundary noise.
> * **RFC 9110 hop-by-hop hardening**: every header listed in `Connection:`
>   is stripped before forwarding (`X-Auth-Internal` smuggling fixed).
> * **Single-pass body inspection** replaces the quadratic per-chunk loop.
> * **Rate-limiter overhaul**: IP keys are canonicalised through `IpAddr`
>   (`::ffff:1.2.3.4` ↔ `1.2.3.4` collapse to one counter), the clock is
>   now monotonic (`Instant`), and shard eviction is **admission control**:
>   when a shard is full incumbents are kept and the *new* arrival is
>   dropped, denying single-IP flood victories.
> * **RFC 7239 `Forwarded:` parser** + `X-Real-IP` fallback.
> * **TLS 1.3 only**, ALPN advertises `h2` + `http/1.1`, 0-RTT explicitly
>   pinned off, key-file permissions checked at startup.
> * **`ArcSwap`** replaces the rule-snapshot `RwLock` (wait-free reads).
> * **`hyper`/`hyper-util` HTTP/2 enabled** at the listener.
> * **`flatten_headers` preserves bytes** — binary header values are no
>   longer dropped to the literal `<binary>` placeholder.
> * **Aho-Corasick now walks every overlap** so additive-score rules
>   accumulate properly.
> * **`tokio` features trimmed** from `["full"]` to a focused subset,
>   reducing binary size and compile-time surface.
