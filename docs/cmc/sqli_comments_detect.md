...
| **No preprocessing** | The detector operates on the raw inspected string — URL decoding and normalisation have already been applied by the engine before CMC inspection. |
...
| Field | Value |
|-------|-------|
| Title | `CMC SQLi comment evasion detection` |
| Severity | `High` |
...
| `rule_match` | `cmc::sqli_comments_detect:comments-total=<N>` |
| `rule_line_match` | `cmc/sqli_comments_detect.rs:generated` |
...
CMC specifically targets the comment-stacking pattern that regex engines often miss
...
2. **Disable the module**: set `SQLi_comments_detect: false` in the CMC config.
