...
Add `Anti_passwd_leak: true` to your DFA config file (default location
...
| Field | Value |
|-------|-------|
| Title | `DFA passwd/shadow file leak detection` |
| Severity | `Critical` |
...
| `rule_match` | `dfa::anti_passwd_leak:passwd:token_a=<a> token_b=<b> count=<N>` |
| `rule_line_match` | `dfa/anti_passwd_leak.rs:generated` |
...
| Reference | [OWASP – Sensitive Data Exposure](https://owasp.org/www-community/vulnerabilities/Sensitive_Data_Exposure) |
| `rule_match` | `dfa::anti_passwd_leak:shadow:token_a=<a> token_b=<b> count=<N>` |
| `rule_line_match` | `dfa/anti_passwd_leak.rs:generated` |
...
2. **Disable the module**: set `Anti_passwd_leak: false` in the DFA config.
...
| File | Purpose |
|------|---------|
| `src/dfa/anti_passwd_leak.rs` | Detector, `PASSWD_TOKENS`, `SHADOW_TOKENS`, `MultiMatcher`, Aho-Corasick/Vectorscan backends, unit tests |
| `src/dfa/mod.rs` | Module registration, `DfaConfig` field, `DfaManager` field, `inspect_response_body()` entry point |
| `src/waf/engine.rs` | Integration — `inspect_response_body()` called at the end of `inspect_response()` |
| `rules/dfa/config.yaml` | Default config — `Anti_passwd_leak: true` |
