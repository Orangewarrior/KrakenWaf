...
on the server — escalating from reflected-XSS to RCE in a single step.  The DFA
...
| **No preprocessing** | URL decoding and case normalisation have already been applied by the engine; the DFA operates on the ready-to-inspect string. |
...
Add `SSTI_detect: true` to your DFA config file (default location
...
| `rule_match` | `dfa::ssti_detect:<id>:<pattern>` |
| `rule_line_match` | `dfa/ssti_detect.rs:generated` |
...
2. **Disable the module**: set `SSTI_detect: false` in the DFA config.
...
| File | Purpose |
|------|---------|
| `src/dfa/ssti_detect.rs` | Detector implementation, rule enum, bounded-search helpers, unit tests |
...
| `rules/dfa/config.yaml` | Default config — `SSTI_detect: true` |
