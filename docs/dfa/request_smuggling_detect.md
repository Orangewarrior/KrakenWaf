...
Add `Request_Smuggling_detect: true` to your DFA config file (default location
...
| Field | Value |
|-------|-------|
| Title | `DFA request smuggling detection` |
| Severity | `High` |
...
| `rule_match` | `dfa::request_smuggling_detect:<pattern>` where `<pattern>` is `transfer-encoding:chunked`, `x-session-hijack:true`, or `content-length<=4` |
| `rule_line_match` | `dfa/request_smuggling_detect.rs:generated` |
...
2. **Disable the module**: set `Request_Smuggling_detect: false` in the DFA config.
...
| File | Purpose |
|------|---------|
| `src/dfa/request_smuggling_detect.rs` | Detector implementation, boundary helpers, unit tests |
...
| `rules/dfa/config.yaml` | Default config — `Request_Smuggling_detect: true` |
