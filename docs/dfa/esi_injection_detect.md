...
Add `ESI_injection_detect: true` to your DFA config file (default location
...
| Field | Value |
|-------|-------|
| Title | `DFA ESI injection detection` |
| Severity | `High` |
...
| `rule_match` | `dfa::esi_injection_detect:<matched-label>` |
| `rule_line_match` | `dfa/esi_injection_detect.rs:generated` |
...
2. **Disable the module**: set `ESI_injection_detect: false` in the DFA config.
...
| File | Purpose |
|------|---------|
| `src/dfa/esi_injection_detect.rs` | Detector implementation, directive table, unit tests |
...
| `rules/dfa/config.yaml` | Default config — `ESI_injection_detect: true` |
