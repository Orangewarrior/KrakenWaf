...
- **Request side** (`inspect_java_deser`): inspects the full HTTP request
  ...
  `inspect_complete_payload_with_context()` after the DFA injection-pattern
...
new `global-options` top-level key in the DFA config file:
...
Add `Java_deserialize_detect: true` under `DFA-Rules` in your config file
(default location `rules/dfa/config.yaml`):
...
| Field | Value |
|-------|-------|
| Title | `DFA Java deserialization attack detection` |
| Severity | `Critical` |
...
| `rule_match` | `dfa::java_deserialize_detect:signals=<A+B+C> evidence=<detail>` |
| `rule_line_match` | `dfa/java_deserialize_detect.rs:generated` |
...
| File | Purpose |
|------|---------|
| `src/dfa/java_deserialize_detect.rs` | `SIGNAL_A_TEXT`, `SIGNAL_B_TEXT`, `SIGNAL_C_TEXT`, `JAVA_DESER_BINARY_MAGIC`, `SingleMatcher`, `JavaDeserializeDfa`, `JavaDeserDecision`, Aho-Corasick/Vectorscan backends, unit tests |
| `src/dfa/mod.rs` | Module registration, `DfaConfig` fields (`java_deserialize_detect`, `untrust_level`), `DfaManager` field, `inspect_java_deser()`, `parse_lenient_yaml()` extended with `global-options` |
...
| `rules/dfa/config.yaml` | Default config — `global-options.Untrust: 60`, `Java_deserialize_detect: true` |
