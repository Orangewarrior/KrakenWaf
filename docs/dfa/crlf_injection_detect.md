...
Add `CRLF_injection_detect: true` to your DFA config file (default location
...
| Field | Value |
|-------|-------|
| Title | `DFA CRLF injection detection` |
| Severity | `High` |
...
| `rule_match` | `dfa::crlf_injection_detect:<vector>` where `<vector>` is `control-line-header`, `escaped-line-header`, or `unicode-line-header` |
| `rule_line_match` | `dfa/crlf_injection_detect.rs:generated` |
...
2. **Disable the module**: set `CRLF_injection_detect: false` in the DFA config.
...
| File | Purpose |
|------|---------|
| `src/dfa/crlf_injection_detect.rs` | Detector, escape table, Unicode table, framing helpers, unit tests |
...
| `rules/dfa/config.yaml` | Default config — `CRLF_injection_detect: true` |
