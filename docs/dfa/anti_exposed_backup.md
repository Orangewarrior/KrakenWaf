...
Add `Anti_exposed_backup: true` to your DFA config file (default location
...
| Field | Value |
|-------|-------|
| Title | `DFA exposed backup/temp file detection` |
| Severity | `High` |
...
| `rule_match` | `dfa::anti_exposed_backup:suffix=<matched_suffix>` |
| `rule_line_match` | `dfa/anti_exposed_backup.rs:generated` |
...
2. **Disable the module**: set `Anti_exposed_backup: false` in the DFA config.
...
| File | Purpose |
|------|---------|
| `src/dfa/anti_exposed_backup.rs` | Detector implementation, Vectorscan helper, unit tests |
| `src/dfa/mod.rs` | Module registration, `DfaConfig` field, `DfaManager` field, `inspect_uri()` |
| `src/waf/engine.rs` | Integration — `inspect_uri()` called in `inspect_early()` |
| `rules/dfa/config.yaml` | Default config — `Anti_exposed_backup: true` |
