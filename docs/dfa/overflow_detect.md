...
Add `Overflow_detect: true` to your DFA config file (default location
...
| Field | Value |
|-------|-------|
| Title | `DFA shellcode opcode detection` |
| Severity | `High` |
...
| `rule_match` | `dfa::overflow_detect:shellcode:<arch>:<pattern> score=<N>` |
| `rule_line_match` | `dfa/overflow_detect.rs:generated` |

### Repeated-character finding

| Field | Value |
|-------|-------|
| Title | `DFA repeated-character overflow detection` |
| Severity | `Medium` |
...
| `rule_match` | `dfa::overflow_detect:repeated-char=<ch> count=<N>` |
| `rule_line_match` | `dfa/overflow_detect.rs:generated` |
...
2. **Disable the module**: set `Overflow_detect: false` in the DFA config.
...
| File | Purpose |
|------|---------|
| `src/dfa/overflow_detect.rs` | Detector implementation, opcode tables, unit tests |
| `src/dfa/mod.rs` | Module registration, `DfaConfig` field, `DfaManager` field, `inspect()` calls |
| `src/waf/engine.rs` | Integration — `inspect()` called in the main WAF pipeline |
| `rules/dfa/config.yaml` | Default config — `Overflow_detect: true` |
