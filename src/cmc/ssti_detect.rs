...
/// The input is already ASCII-lowercased by the engine before reaching the CMC.
...
#[cfg(test)]
mod tests {
    use super::{SstiCmcBuilder, SstiRule};

    #[test]
    fn detects_common_ssti_families() {
        let cmc = SstiCmcBuilder::new().build();

        assert!(matches!(cmc.detect("{{7*7}}"), Some(SstiRule::DoubleCurly)));
        assert!(matches!(
            cmc.detect("{% debug %}"),
            Some(SstiRule::PercentBlock)
        ));
        assert!(matches!(
            cmc.detect("<#assign x=7>"),
            Some(SstiRule::FreeMarkerDirective)
        ));
        assert!(matches!(
            cmc.detect("#set($x = 7 * 7)"),
            Some(SstiRule::VelocitySet)
        ));
        assert!(matches!(
            cmc.detect("[[user.name]]"),
    ...
    fn detects_extended_ssti_patterns() {
        ...
        assert!(matches!(
            cmc.detect("[#list items as item][/#list]"),
        ...
        assert!(matches!(
            cmc.detect("#foreach($item in $list)#end"),
            Some(SstiRule::VelocityForeach)
        ));
        assert!(matches!(
            cmc.detect("#if($user == 'admin')owned#end"),
            Some(SstiRule::VelocityIf)
        ));
        assert!(matches!(
            cmc.detect("~{fragments/nav :: menu}"),
