...
pub struct NoSqlInjectionCmcBuilder {
    vectorscan_enabled: bool,
}

#[derive(Debug, Clone)]
pub struct NoSqlInjectionCmc {
...
impl LiteralMatcher {
    fn new(patterns: &'static [&'static str], vectorscan_enabled: bool) -> Self {
        let ac = AhoCorasickBuilder::new()
            ...
            .build(patterns)
            .expect("static NoSQL CMC patterns must compile");
...
#[cfg(test)]
mod tests {
    use super::NoSqlInjectionCmcBuilder;

    #[test]
    fn requires_one_marker_from_each_list() {
        let cmc = NoSqlInjectionCmcBuilder::new().build();
        ...
        assert!(cmc.detect(r#"{"$gt":""}"#).is_none());
        assert!(cmc.detect(r#"{"user":"admin"}"#).is_none());
    }
    ...
    fn detects_numeric_equality_as_list_b() {
        ...
        assert!(cmc.detect(r#"{"$where":"this.age==7"}"#).is_some());
        assert!(cmc.detect(r#"{"$where":"this.age== 9"}"#).is_some());
        assert!(cmc.detect(r#"{"$where":"this.age==0"}"#).is_none());
    }
}
