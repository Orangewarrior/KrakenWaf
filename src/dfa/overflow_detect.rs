#[derive(Debug, Clone)]
pub struct OverflowDfaBuilder {
    threshold: usize,
}
#[derive(Debug, Clone)]
pub struct OverflowDfa {
    threshold: usize,
}
...
#[cfg(test)]
mod tests {
    use super::{OverflowDfaBuilder, ShellcodeArch};

    #[test]
    fn detects_structured_overflow_patterns() {
        let dfa = OverflowDfaBuilder::new().threshold(10).build();

        assert!(dfa.detect_run("AAAAAAAAAAAA").is_some());
        assert!(dfa.detect_run("%n%n%n%n%n").is_some());
        assert!(dfa.detect_run("123456789012345678901234567890").is_some());
        assert!(dfa.detect_run("../../../../../../etc/passwd").is_some());
    }
    ...
    fn detects_common_x86_shellcode_opcodes() {
        ...
        let nop = dfa.detect_shellcode(nop_sled).expect("x86 nop sled");
        assert_eq!(nop.arch(), ShellcodeArch::X86);

        let shellcode = dfa.detect_shellcode(execve).expect("x86 execve shellcode");
    ...
    fn detects_common_x64_shellcode_opcodes() {
        ...
        let shellcode = dfa.detect_shellcode(payload).expect("x64 shellcode");
    ...
    fn detects_common_arm_and_thumb_shellcode_opcodes() {
        ...
        let shellcode = dfa.detect_shellcode(arm).expect("arm shellcode");
        assert_eq!(shellcode.arch(), ShellcodeArch::Arm);

        let sled = dfa.detect_shellcode(thumb_nop).expect("thumb nop sled");
    ...
    fn parses_percent_and_0x_encoded_shellcode_bytes() {
        ...
        assert!(dfa.detect_shellcode("%90%90%90%90%cc").is_some());
        assert!(dfa.detect_shellcode("0x6a0x3b0x580x0f0x05").is_some());
    }
}
