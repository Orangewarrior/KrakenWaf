...
pub struct RequestSmugglingDfaBuilder;
#[derive(Debug, Clone)]
pub struct RequestSmugglingDfa;
...
impl RequestSmugglingDfaBuilder {
    pub fn new() -> Self {
        Self
    }

    pub fn build(self) -> RequestSmugglingDfa {
        RequestSmugglingDfa
    }
}

impl RequestSmugglingDfa {
...
#[cfg(test)]
mod tests {
    use super::RequestSmugglingDfaBuilder;

    #[test]
    fn detects_smuggling_headers_and_body_injections() {
        let dfa = RequestSmugglingDfaBuilder::new().build();

        assert!(dfa
            .detect("get / http/1.1\r\ntransfer-encoding: chunked\r\n\r\n")
            .is_some());
        assert!(dfa.detect("payload=transfer-encoding: chunked").is_some());
        assert!(dfa.detect("payload=x-session-hijack: true").is_some());
        assert!(dfa
    ...
    fn ignores_larger_content_length_and_non_matching_values() {
        ...
        assert!(dfa
            .detect("post / http/1.1\r\ncontent-length: 20\r\n\r\nsafe body")
            .is_none());
        assert!(dfa.detect("payload=transfer-encoding: gzip").is_none());
        assert!(dfa.detect("payload=x-session-hijack: false").is_none());
    }
}
