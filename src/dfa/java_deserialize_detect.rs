...
/// Decision returned by `JavaDeserializeDfa::detect`.
#[derive(Debug, Clone)]
...
/// Builder for `JavaDeserializeDfa`.
#[derive(Debug, Clone)]
pub struct JavaDeserializeDfaBuilder {
    untrust_level: u8,
    vectorscan_enabled: bool,
}

impl Default for JavaDeserializeDfaBuilder {
    fn default() -> Self {
        Self {
            untrust_level: 60,
...
impl JavaDeserializeDfaBuilder {
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub fn build(self) -> JavaDeserializeDfa {
        JavaDeserializeDfa {
...
mod tests {
    ...
    fn dfa_default() -> JavaDeserializeDfa {
        JavaDeserializeDfaBuilder::new().build()
    }

    fn dfa_untrust(level: u8) -> JavaDeserializeDfa {
        JavaDeserializeDfaBuilder::new().untrust_level(level).build()
    }
    ...
    fn three_signals_always_block() {
        let d = dfa_untrust(0); // even untrust=0 should not matter for 3 signals
        let text = "Content-Type: application/x-java-serialized-object\n\nrO0AAABw";
