...
pub struct XxeAttackDfaBuilder {
    vectorscan_enabled: bool,
}

#[derive(Debug, Clone)]
pub struct XxeAttackDfa {
...
impl XxeAttackDfaBuilder {
    ...
    pub fn build(self) -> XxeAttackDfa {
        XxeAttackDfa {
...
impl LiteralMatcher {
    fn new(patterns: &'static [&'static str], vectorscan_enabled: bool) -> Self {
        let ac = AhoCorasickBuilder::new()
            ...
            .build(patterns)
            .expect("static XXE DFA patterns must compile");
...
#[cfg(test)]
mod tests {
    use super::XxeAttackDfaBuilder;

    #[test]
    fn requires_one_marker_from_each_list() {
        let dfa = XxeAttackDfaBuilder::new().build();
        ...
        assert!(dfa.detect(r#"<!ENTITY harmless "value">"#).is_none());
        assert!(dfa.detect(r#"<data>file:///etc/passwd</data>"#).is_none());
    }
    ...
    fn detects_utf16le_encoded_payload_after_url_decode() {
        ...
        let matched = dfa.detect(&payload).expect("utf16le xxe should match");
