...
pub struct AntiPasswdLeakDfaBuilder {
    vectorscan_enabled: bool,
}

#[derive(Debug, Clone)]
pub struct AntiPasswdLeakDfa {
    passwd: MultiMatcher,
    shadow: MultiMatcher,
}
...
impl AntiPasswdLeakDfaBuilder {
    ...
    pub fn build(self) -> AntiPasswdLeakDfa {
        AntiPasswdLeakDfa {
...
impl MultiMatcher {
    fn new(patterns: &'static [&'static str], threshold: usize, vectorscan_enabled: bool) -> Self {
        let ac = AhoCorasickBuilder::new()
            ...
            .build(patterns)
            .expect("static passwd-leak DFA patterns must compile");
...
#[cfg(test)]
mod tests {
    use super::{AntiPasswdLeakDfaBuilder, LeakKind};

    fn dfa() -> super::AntiPasswdLeakDfa {
        AntiPasswdLeakDfaBuilder::new().build()
    }
