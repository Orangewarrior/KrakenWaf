...
async fn build_engine(vectorscan_enabled: bool) -> WafEngine {
...
#[tokio::test(flavor = "multi_thread")]
async fn blocks_common_dvwa_post_xss_probe() {
    let engine = build_engine(false).await;
...
#[tokio::test(flavor = "multi_thread")]
async fn blocks_common_dvwa_post_sqli_probe() {
    let engine = build_engine(false).await;
...
#[tokio::test(flavor = "multi_thread")]
async fn vectorscan_engine_blocks_fast_literals() {
...
#[cfg(feature = "vectorscan-engine")]
#[tokio::test(flavor = "multi_thread")]
async fn vectorscan_engine_blocks_form_urlencoded_plus_payloads() {
    let engine = build_engine(true).await;
...
#[cfg(feature = "vectorscan-engine")]
#[tokio::test(flavor = "multi_thread")]
async fn vectorscan_engine_blocks_long_post_payload_beyond_old_overlap_window() {
    let engine = build_engine(true).await;
...
#[tokio::test(flavor = "multi_thread")]
async fn blocks_common_dvwa_get_sqli_probe_via_full_payload() {
    let engine = build_engine(false).await;
...
#[tokio::test(flavor = "multi_thread")]
async fn lowercases_before_matching_post_payloads() {
    let engine = build_engine(false).await;
    let body = br"txtName=%3CSCRIPT%3Ealert(1)%3C%2FSCRIPT%3E&mtxMessage=owned";
    assert!(matches!(engine.inspect_complete_payload(body), Decision::Block(_)));
}
